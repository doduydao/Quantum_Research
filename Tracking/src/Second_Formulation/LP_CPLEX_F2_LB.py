...
if __name__ == '__main__':
    ...
    data_path = src_path + '/6hits/know_track/hits.csv'
    hits_volume = read_hits(data_path)
    hits = hits_volume[9]

    out_angles_path = "results/6hits/min_angles.json"
    angles = calculate_all_angles(hits, out_angles_path)
    LB = sum(list(angles.values()))

    model_path_out = "results/6hits/know_track/model_docplex_LB.lp"
    solution_path_out = "results/6hits/know_track/solution.json"
    figure_path_out = "results/6hits/know_track/result.PNG"
